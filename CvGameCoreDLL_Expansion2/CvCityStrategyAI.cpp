		...
		if(iBldgLoop != eIgnoreBldg && m_pCity->canConstruct(eLoopBuilding))
		{
			...
			if (MOD_AI_SMART_V3)
			{
				...
				if((kbClassInfo.getMaxPlayerInstances() == 1) || (kbClassInfo.getMaxGlobalInstances() == 1))
				{
					...
					bool bIsCultureRelatedBuilding = (pkBuildingInfo->GetCultureRateModifier() > 0) || ((pkBuildingInfo->GetLandmarksTourismPercent() + pkBuildingInfo->GetGreatWorksTourismModifier() + pkBuildingInfo->GetLandmarksTourismPercentGlobal() + pkBuildingInfo->GetGreatWorksTourismModifierGlobal() + pkBuildingInfo->GetLandmarksTourismPerXForeignFollowers()) > 0);
