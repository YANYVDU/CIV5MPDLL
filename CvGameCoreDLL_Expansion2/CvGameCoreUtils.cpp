...
bool IsPromotionValidForUnitPromotions(PromotionTypes ePromotion, CvUnit& pUnit)
{
	...
	if(!pExclusions.empty())
	{
		...
		{
			if(prePromotions[Ii] != NO_PROMOTION && pUnit.isHasPromotion((PromotionTypes)pExclusions[Ii]))
			return false;
		}
	}
	...
	if(!pPrereqAnds.empty())
	{
		...
		{
			if(prePromotions[Ii] != NO_PROMOTION && !pUnit.isHasPromotion((PromotionTypes)pPrereqAnds[Ii]))
			return false;
		}
	}
