...
int CustomMods::getOption(string sOption, int defValue) {
	if (!m_bInit) {
		...
		MOD_OPT_CACHE(PROMOTION_INSTANT_MOVES);
		...
		MOD_OPT_CACHE(GLOBAL_AI_PERF_LOGGING);
		...
		MOD_OPT_CACHE(EVENTS_PLAYER_SET_HAS_TECH);
		...
		MOD_OPT_CACHE(ROG_CORE_BOMBARD_RANGE_BUILDINGS);
		...
		MOD_OPT_CACHE(VOLCANO_BREAK);
		MOD_OPT_CACHE(ERA_EFFECTS_EXTENSIONS);

		MOD_OPT_CACHE(TRAIN_ALL_CORE);
		MOD_OPT_CACHE(ROG_CORE);
		MOD_OPT_CACHE(DISEASE_BREAK);
		MOD_OPT_CACHE(IMPROVEMENT_FUNCTION);
		MOD_OPT_CACHE(BUILDING_IMPROVEMENT_RESOURCES);
		...
		MOD_OPT_CACHE(EVENTS_DO_DENOUNCE);
		...
		MOD_OPT_CACHE(AIR_SWEEP_FOUND_SOMETHING);
		MOD_OPT_CACHE(SHOW_ATTACK_INTERFACE_WITHOUT_ENEMY);
		...
		MOD_OPT_CACHE(PROMOTION_NEW_EFFECT_FOR_SP);
		MOD_OPT_CACHE(PROMOTION_AURA_PROMOTION);
		MOD_OPT_CACHE(POLICY_FREE_PROMOTION_FOR_PROMOTION);
		MOD_OPT_CACHE(PROMOTION_FEATURE_INVISIBLE);
		MOD_OPT_CACHE(GLOBAL_TRIGGER_NEW_GOLDEN_AGE_IN_GA);
		MOD_OPT_CACHE(POLICY_NEW_EFFECT_FOR_SP);
		MOD_OPT_CACHE(GLOBAL_UNIT_MOVES_AFTER_DISEMBARK);
		MOD_OPT_CACHE(GLOBAL_INCREASE_SS_PART_PURCHASE_COST);
		MOD_OPT_CACHE(GLOBAL_RANGE_ATTACK_KILL_POPULATION_OF_HEAVY);
		MOD_OPT_CACHE(GLOBAL_CAPTURE_UNIT_CANNOT_MAX_OUT);
		MOD_OPT_CACHE(GLOBAL_BUILDING_INSTANT_YIELD);
		MOD_OPT_CACHE(BUILDING_NEW_EFFECT_FOR_SP);
		MOD_OPT_CACHE(VOTING_NEW_EFFECT_FOR_SP);
		MOD_OPT_CACHE(BELIEF_NEW_EFFECT_FOR_SP);
		MOD_OPT_CACHE(TRAIT_NEW_EFFECT_FOR_SP);
		MOD_OPT_CACHE(TRAIT_WOOD_AS_ROAD_SP);
		MOD_OPT_CACHE(TROOPS_AND_CROPS_FOR_SP);
		MOD_OPT_CACHE(INTERNATIONAL_IMMIGRATION_FOR_SP);
		MOD_OPT_CACHE(NUCLEAR_WINTER_FOR_SP);

		MOD_OPT_CACHE(UNIT_COST_DONOT_INCTEASE_WITH_TURN);
		MOD_OPT_CACHE(NO_FASTER_REPAIR_OUTSIDE);
		MOD_OPT_CACHE(GLOBAL_MAX_PLOT_BUILD);
		MOD_OPT_CACHE(CITY_NO_DECAY);
		MOD_OPT_CACHE(CHANGE_RESOURCE_LINK_AFTER_ALTER_PLOT);

		MOD_OPT_CACHE(GLOBAL_CITY_SCALES);
		MOD_OPT_CACHE(EVENTS_CITY_SCALES);

		MOD_OPT_CACHE(EVENTS_INTERNATIONAL_IMMIGRATION);
		...
		MOD_OPT_CACHE(SP_FASTER_AI);
		MOD_OPT_CACHE(SP_UNIQUE_CITYSTATE);
		MOD_OPT_CACHE(SP_CITYSTATE_BASIC);
		MOD_OPT_CACHE(LUXURY_ERA_DECAY);

		MOD_OPT_CACHE(TRAITS_SPREAD_RELIGION_AFTER_KILLING);
		MOD_OPT_CACHE(TRAITS_COMBAT_BONUS_FROM_CAPTURED_HOLY_CITY);
		MOD_OPT_CACHE(TRAITS_SIEGE_BONUS_IF_SAME_RELIGION);
		MOD_OPT_CACHE(TRAITS_ENABLE_FAITH_PURCHASE_ALL_COMBAT_UNITS);
		...
		MOD_OPT_CACHE(GLOBAL_CORRUPTION_POLICY_COST_MIN_ZERO);
		MOD_OPT_CACHE(GLOBAL_UNIT_BARBARIAN_CAN_TRAIN);
		MOD_OPT_CACHE(GLOBAL_SP_BARBARIAN_ENHANCE);
		MOD_OPT_CACHE(GLOBAL_UNIQUE_PROJECT_CAPTURE);
		MOD_OPT_CACHE(GLOBAL_HOLY_CITY_FOUNDER_CHANGE);
		MOD_OPT_CACHE(GLOBAL_OPTIONAL_UC);
